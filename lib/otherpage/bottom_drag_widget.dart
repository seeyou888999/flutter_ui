...
class BottomDragfulWidget extends StatefulWidget {
  final Widget body;
  final DragContainer dragContainer;
  BottomDragfulWidget({Key key, @required this.body, @required this.dragContainer})
      :
        assert(dragContainer != null),
        super(key: key);

  @override
  _BottomDragfulWidgetState createState() => _BottomDragfulWidgetState();
}

class _BottomDragfulWidgetState extends State<BottomDragfulWidget> {
  @override
...
class _DragContainerState extends State<DragContainer>
  ...
  void initState() {
    ...
    maxOffsetDistance = (widget.height + widget.defaultShowHeight) * 0.5; //偏移隐藏标准
//    if (controller != null) {
    _controller
